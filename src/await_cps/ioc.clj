...
(defn has-terminators?
  ...
  (let [sym (when (seq? form) (first form))]
    (cond
      (contains? terminators (var-name env sym)) true
      (and recur-target (= 'recur sym))          true
      (= 'loop* sym)
      (some #(has-terminators? % (dissoc ctx :recur-target)) (rest form))
      (coll? form)
      (some #(has-terminators? % ctx) form)
      :else false)))
...
(defn invert
  [{:keys [r e sync-recur? recur-target terminators env] :as ctx}
   form]
    ...
    (cond
      ...
      (let [handler (terminators (var-name env head))]
        (resolve-sequentially ctx (rest form)
          (fn [args]
            `(letfn [(safe-r# [v#] (try (~r v#) (catch ~all-ex t# (~e t#))))] ; all-ex is :default in CLJS
